...
请以JSON格式返回结果。
      ...
      const systemPrompt = `你是一位经验丰富的教育专家和课程设计师。请根据用户提供的信息生成完整的课程大纲，并以JSON格式返回。确保返回的是有效的JSON数据。`;
...
请开始生成高质量的测验题目。
      ...
      const systemPrompt = `你是一位经验丰富的教育测评专家和题库设计师。请严格按照要求生成高质量的测验题目，并以JSON格式返回。确保返回的是有效的JSON数据，包含questions数组和metadata对象。`;
...
请确保生成的题目具有独特性和创新性！
      ...
      const systemPrompt = `你是一位经验丰富的教育测评专家。请生成一道全新的、具有创新性的测验题目，确保与已有题目不重复，并以JSON格式返回。`;
...
请以JSON格式返回结果：
{
  ...
  "resources": ["参考资源1", "参考资源2"]
      ...
      const systemPrompt = `你是一位经验丰富的教育专家。请根据用户要求生成详细的作业模板，并以JSON格式返回。`;
  ...
  "resources": [
    {
      ...
      const systemPrompt = `你是一位经验丰富的教育资源专家。请根据用户要求推荐高质量的教学资源，并以JSON格式返回。`;
